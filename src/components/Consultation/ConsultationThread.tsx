...
import { Consultation, Response, User as UserType } from '../../types';
...
export const ConsultationThread: React.FC<ConsultationThreadProps> = ({
  ...
  const getMessageSource = (response: Response) => {
    ...
    return { icon: Smartphone, label: 'Mobile App', color: 'text-purple-400' };
  };
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex flex-col">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="text-lg font-semibold">{consultation.title}</h1>
              <span className={`px-2 py-1 rounded text-xs ${
                ...
                consultation.status === 'resolved' ? 'bg-green-900/20 text-green-400' :
            ...
            <div className="flex items-center gap-4 text-sm text-gray-400">
              ...
              <span>•</span>
              <span>{consultation.responses.length} responses</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between text-xs">
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-400">
              ...
              {connectionStatus.isOnline ? ' Real-time sync' : ' Will sync when connected'}
            </span>
          </div>
          {consultation.status !== 'resolved' && currentUser.role === 'specialist' && (
            <button
              onClick={() => {
                onMarkResolved();
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className={`border rounded-lg p-4 ${getPriorityColor(consultation.priority)}`}>
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                <span className="font-medium">Dr. {consultation.createdByName || 'Field Medic'}</span>
                <span className="text-xs text-gray-400">•</span>
                <span className="text-xs text-gray-400">{formatTime(consultation.createdAt)}</span>
                <div className="flex items-center gap-1 text-xs">
                  {consultation.source === 'whatsapp' ? (
                    ...
                    <>
                      ...
                      <span className="text-blue-400">Web App</span>
                    </>
                  )}
                </div>
              </div>
                ...
                <div className="mt-3 space-y-2">
                  {consultation.attachments.map((attachment, index) => (
                    <div key={index} className="bg-gray-700/50 rounded p-2 text-sm">
                      ...
                      <button
                        onClick={() => notImplemented('View Attachment')}
        ...
        {consultation.responses.map((response, index) => {
          ...
          return (
            <div key={response.id} className={`flex ${isOwnMessage ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] ${isOwnMessage ? 'order-2' : 'order-1'}`}>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-sm font-medium">
                    {isOwnMessage ? 'You' : `Dr. ${response.userName}`}
                  </span>
                  <span className="text-xs text-gray-400">•</span>
                  <span className="text-xs text-gray-400">{formatTime(response.createdAt)}</span>
                  ...
                  {response.isOffline && (
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-xs text-yellow-400">Pending sync</span>
                    </div>
                  )}
                </div>
        ...
        {connectionStatus.isOnline && consultation.status === 'in_progress' && (
        ...
        <div className="bg-gray-800 border-t border-gray-700 p-4 flex-shrink-0">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center justify-between mt-2">
                  <div className="flex items-center gap-4 text-xs text-gray-400">
                    <span>
                      {connectionStatus.isOnline ? 'Will send immediately' : 'Will send when connected'}
                    </span>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      onClick={() => notImplemented('Voice Response')}
                    ...
                    <button
                      type="button"
                      onClick={() => notImplemented('Add Attachment')}
                      className="p-2 text-gray-400 hover:text-white transition-colors"
          ...
          {currentUser.role === 'specialist' && (
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                onClick={() => setNewResponse('Please provide more details about the patient\'s vital signs and current symptoms.')}
                className="px-3 py-1 bg-gray-700 hover:bg-gray-600 rounded text-xs transition-colors"
              >
                Request More Info
              </button>
              <button
                onClick={() => setNewResponse('Based on the symptoms described, I recommend immediate...')}
                className="px-3 py-1 bg-gray-700 hover:bg-gray-600 rounded text-xs transition-colors"
              >
                Emergency Protocol
              </button>
              <button
                onClick={() => setNewResponse('This appears to be a stable condition. Monitor for...')}
                className="px-3 py-1 bg-gray-700 hover:bg-gray-600 rounded text-xs transition-colors"
              >
                Monitoring Advice
              </button>
      ...
      {consultation.status === 'resolved' && (
        <div className="bg-green-900/20 border-t border-green-500/30 p-4 flex items-center justify-center gap-2">
          ...
          <span className="text-green-400 font-medium">This consultation has been resolved</span>
        </div>
      )}
    </div>
  );
