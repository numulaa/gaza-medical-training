...
export const ConsultingDoctorDashboard: React.FC<ConsultingDoctorDashboardProps> = ({
  user,
  connectionStatus,
  onLogout,
  notImplemented
  ...
  const [activeTab, setActiveTab] = useState<'my_questions' | 'received'>('my_questions');
  const [joinCode, setJoinCode] = useState('');
  const [sortBy, setSortBy] = useState<'date' | 'priority' | 'specialty' | 'responses'>('date');
  const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('desc');
  const [filterPriority, setFilterPriority] = useState<'all' | 'emergency' | 'urgent' | 'standard'>('all');
  ...
  const [consultations] = useState<Consultation[]>([
    {
      ...
      description: 'Shrapnel wound to chest, difficulty breathing, BP 90/60',
    ...
    },
    {
      ...
      responses: [{ 
        ...
        userId: 'spec1', 
        userName: 'Dr. Smith', 
        content: 'Please check CBC and blood culture. Monitor for signs of dehydration.', 
    ...
    },
    {
      ...
      description: 'Patient with chest pain and irregular heartbeat, need cardiology input',
    ...
    },
    {
      ...
      description: 'Abdominal trauma, possible internal bleeding, need surgical assessment',
      specialty: 'Surgery',
      priority: 'emergency',
      status: 'in_progress',
      ...
      responses: [{ 
        ...
        userId: user.id, 
        userName: user.name, 
        content: 'Based on description, recommend immediate surgical exploration. Prepare for emergency laparotomy.', 
        createdAt: new Date(Date.now() - 5 * 60 * 1000),
        source: 'web'
      }],
      source: 'web'
  ...
  const receivedConsultations = consultations.filter(c => c.createdBy !== user.id);
  ...
  const filteredReceivedConsultations = applyFiltersAndSort(receivedConsultations);
        ...
        <div className="space-y-4">
          ...
          <div className="bg-gray-800 rounded-lg p-4 space-y-4">
            ...
            <div className="text-xs text-gray-400">
              Showing {activeTab === 'my_questions' ? filteredMyQuestions.length : filteredReceivedConsultations.length} of {activeTab === 'my_questions' ? myQuestions.length : receivedConsultations.length} consultations
            </div>
          </div>
          ...
          {activeTab === 'my_questions' && (
            <div className="space-y-4">
              {filteredMyQuestions.length === 0 ? (
                ...
                filteredMyQuestions.map(consultation => renderConsultationCard(consultation, true))
              )}
            </div>
          )}
                ...
                <div className="bg-gray-800 border border-gray-700 rounded-lg p-6 text-center">
                  ...
                  {receivedConsultations.length === 0 && (
                    <p className="text-sm text-gray-500 mt-2">
                      Questions you join with codes or links will appear here.
                    </p>
                  )}
                </div>
              ) : (
                filteredReceivedConsultations.map(consultation => renderConsultationCard(consultation, false))
              )}
            </div>
          )}
        </div>
