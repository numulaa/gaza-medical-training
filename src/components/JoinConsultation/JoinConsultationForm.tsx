...
import { Stethoscope, ArrowLeft, Hash } from 'lucide-react';
...
export const JoinConsultationForm: React.FC<JoinConsultationFormProps> = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!success) {
        setError('Invalid consultation code. Please check and try again.');
      }
    } catch (error) {
  ...
  const handleCodeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (formatted.replace(/-/g, '').length <= 9) { // Max 9 characters (3 groups of 3)
      setCode(formatted);
    }
  };
      ...
      <div className="w-full max-w-md bg-gray-800 rounded-lg p-8 shadow-lg">
        <div className="flex items-center mb-6">
          ...
          <div className="flex items-center">
            ...
            <h1 className="text-xl font-bold text-white">Join Consultation</h1>
          </div>
        </div>

        <div className="mb-6">
          <p className="text-gray-300 text-sm mb-4">
            Enter the consultation code to join an ongoing medical consultation thread.
          </p>
          <div className="bg-blue-900/20 border border-blue-500/30 rounded-lg p-3">
            <p className="text-blue-300 text-xs">
              💡 Consultation codes are shared by the consulting doctor and allow secure access to specific medical cases.
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 space-y-3">
          <div className="border-t border-gray-700 pt-4">
            ...
            <ul className="text-xs text-gray-400 space-y-1">
              <li>• Consulting doctors share consultation codes</li>
              <li>• Specialists use codes to join specific cases</li>
              <li>• Secure access without browsing all consultations</li>
